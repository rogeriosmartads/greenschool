...
class KindsControllerTest < ActionDispatch::IntegrationTest
  ...
  test "should create kind" do
    assert_difference('Kind.count') do
      post kinds_url, params: { kind: { code: @kind.code, name: @kind.name, status: @kind.status } }
    end
  ...
  test "should update kind" do
    patch kind_url(@kind), params: { kind: { code: @kind.code, name: @kind.name, status: @kind.status } }
